...
enum SDLNativeGameRenderer {
    ...
    private static func drawBattle(_ r: GameRenderer, game: Game, width: Int, height: Int) {
        let field = playfieldBounds(width: Double(width), height: Double(height))
        text(r, "THUNDER SWIFT", 16, 16, RenderColor(225, 242, 255))
        text(r, "STAGE \(game.stage)", 190, 16, RenderColor(126, 190, 255))
        text(r, "KILLS \(game.kills)", Float(width - 170), 16, RenderColor(235, 187, 255))
        text(r, "SCORE \(game.score)", Float(width - 90), 16, RenderColor(255, 219, 125))
        bar(r, x: 18, y: 39, width: 190, value: game.health / max(1, game.maxHealth), fill: RenderColor(239, 70, 105), back: RenderColor(61, 28, 53))
        text(r, "HP \(Int(game.health))/\(Int(game.maxHealth))", 22, 42, RenderColor(255, 244, 247))
        ...
        text(r, "THUNDER \(Int(game.thunderEnergy))%", 430, 42, RenderColor(106, 239, 255))
        if game.combo > 1 { text(r, "COMBO x\(game.combo)", 640, 16, RenderColor(255, 181, 91)) }
        ...
        if let boss = game.boss {
            ...
            text(r, "BOSS  \(BossType(rawValue: boss.kind)?.title ?? "DREADNOUGHT")", Float(width / 2 - 110), 67, RenderColor(255, 225, 255))
        }
    ...
    private static func drawMenu(_ r: GameRenderer, game: Game, width: Int, height: Int) {
        ...
        text(r, "THUNDER SWIFT", Float(width / 2 - 120), 122, RenderColor(230, 245, 255))
        text(r, "NEON SKY // ARCADE FLIGHT SYSTEM", Float(width / 2 - 155), 160, RenderColor(89, 195, 246))
        let buttons = mainMenuButtons(width: Double(width), height: Double(height))
        let labels = ["NEW GAME", "CONTROLS", "HANGAR", "SETTINGS", "EXIT", "ARCHIVE"]
        for i in buttons.indices { button(r, buttons[i], title: labels[i], selected: false) }
        text(r, "CREDITS \(game.profile.credits)   CORES \(game.profile.cores)   ALLOY \(game.profile.alloy)", Float(width / 2 - 170), 250, RenderColor(255, 211, 112))
        text(r, "SAVE SLOT \(SaveManager.shared.activeSlot + 1)  •  AUTO-SAVE ON", 30, 42, RenderColor(137, 238, 180))
    }
    ...
    private static func drawSaveSlots(_ r: GameRenderer, game: Game, width: Int, height: Int) {
        ...
        text(r, "SAVE SELECT", Float(width / 2 - 75), 120, RenderColor(230, 245, 255))
        for (i, card) in saveSlotCards(width: Double(width), height: Double(height)).enumerated() { button(r, card, title: "SAVE SLOT \(i + 1)", selected: i == SaveManager.shared.activeSlot) }
        button(r, saveSlotsBackButton(width: Double(width), height: Double(height)), title: "BACK", selected: false)
    }
    ...
    private static func drawMissionSelect(_ r: GameRenderer, game: Game, width: Int, height: Int) {
        ...
        text(r, "MISSION SELECT", Float(width / 2 - 90), 92, RenderColor(230, 245, 255))
        for (i, card) in missionCards(width: Double(width), height: Double(height)).enumerated() { button(r, card, title: "SECTOR \(i + 1)", selected: i == game.selectedMission) }
        for (i, card) in modeCards(width: Double(width), height: Double(height)).enumerated() { button(r, card, title: GameMode(rawValue: i)?.label ?? "MODE", selected: i == game.gameMode.rawValue) }
        button(r, missionLaunchButton(width: Double(width), height: Double(height)), title: "LAUNCH", selected: true)
        button(r, missionBackButton(width: Double(width), height: Double(height)), title: "BACK", selected: false)
    }
    ...
    private static func drawControls(_ r: GameRenderer, game: Game, width: Int, height: Int) {
        ...
        text(r, "CONTROL DECK", Float(width / 2 - 90), 120, RenderColor(230, 245, 255))
        ...
        button(r, modes[1], title: "MOUSE FOLLOW", selected: game.controlMode == .mouse)
        button(r, controlsBackButton(width: Double(width), height: Double(height)), title: "BACK", selected: false)
    }

    private static func drawSettings(_ r: GameRenderer, game: Game, width: Int, height: Int) {
        panel(r, x: width / 2 - 320, y: 70, width: 640, height: height - 125)
        text(r, "SETTINGS", Float(width / 2 - 55), 120, RenderColor(230, 245, 255))
        let language = settingsLanguageButtons(width: Double(width), height: Double(height))
        button(r, language[0], title: "ENGLISH", selected: game.language == .english)
        button(r, language[1], title: "CHINESE", selected: game.language == .chinese)
        ...
        button(r, settingsShakeButton(width: Double(width), height: Double(height)), title: "CAMERA SHAKE", selected: false)
        button(r, settingsWindowModeButton(width: Double(width), height: Double(height)), title: game.profile.isFullscreen ? "DISPLAY: FULLSCREEN" : "DISPLAY: WINDOWED", selected: false)
        button(r, settingsResolutionButton(width: Double(width), height: Double(height)), title: "RESOLUTION: \(game.profile.resolutionWidth)x\(game.profile.resolutionHeight)", selected: false)
        button(r, settingsBackButton(width: Double(width), height: Double(height)), title: "BACK", selected: false)
    }
    ...
    private static func drawHangar(_ r: GameRenderer, game: Game, width: Int, height: Int) {
        ...
        text(r, "HANGAR / LOADOUT", Float(width / 2 - 120), 90, RenderColor(230, 245, 255))
        for (i, tab) in hangarTabButtons(width: Double(width), height: Double(height)).enumerated() { button(r, tab, title: i == 0 ? "EQUIPMENT" : "VAULT", selected: i == game.hangarTab) }
        for (i, card) in hangarCards(width: Double(width), height: Double(height)).enumerated() { button(r, card, title: "MODULE \(i + 1)", selected: false) }
        button(r, hangarBackButton(width: Double(width), height: Double(height)), title: "BACK", selected: false)
    }
    ...
    private static func drawArchive(_ r: GameRenderer, game: Game, width: Int, height: Int) {
        ...
        text(r, "ARCHIVE", Float(width / 2 - 50), 100, RenderColor(230, 245, 255))
        let tabs = archiveTabButtons(width: Double(width), height: Double(height))
        button(r, tabs[0], title: "ACHIEVEMENTS", selected: game.archiveTab == 0)
        button(r, tabs[1], title: "CODEX", selected: game.archiveTab == 1)
        button(r, archiveBackButton(width: Double(width), height: Double(height)), title: "BACK", selected: false)
    }
    ...
    private static func drawPause(_ r: GameRenderer, game: Game, width: Int, height: Int) {
        ...
        text(r, "MISSION PAUSED", Float(width / 2 - 85), Float(height / 2 - 140), RenderColor(240, 247, 255))
        let buttons = pauseButtons(width: Double(width), height: Double(height))
        for (i, title) in ["RESUME", "RESTART", "SETTINGS", "HANGAR", "EXIT"].enumerated() { button(r, buttons[i], title: title, selected: false) }
    }

    private static func drawUpgrade(_ r: GameRenderer, game: Game, width: Int, height: Int) {
        text(r, "MODULE AVAILABLE", Float(width / 2 - 105), Float(height / 2 - 175), RenderColor(244, 211, 116))
        for (i, card) in upgradeCards(width: Double(width), height: Double(height)).enumerated() { button(r, card, title: i < game.upgradeOptions.count ? game.upgradeOptions[i].title : "MODULE", selected: false) }
    }
    ...
    private static func drawGameOver(_ r: GameRenderer, game: Game, width: Int, height: Int) {
        ...
        text(r, game.runWon ? "MISSION COMPLETE" : "SORTIE FAILED", Float(width / 2 - 90), Float(height / 2 - 120), RenderColor(244, 211, 116))
        text(r, "SCORE  \(game.score)", Float(width / 2 - 55), Float(height / 2 - 45), RenderColor(237, 246, 255))
        let buttons = gameOverButtons(width: Double(width), height: Double(height))
        button(r, buttons[0], title: "RESTART", selected: true)
        button(r, buttons[1], title: "MAIN MENU", selected: false)
    }
